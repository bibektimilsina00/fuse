#!/usr/bin/env bash
# ----------------------------------------------------------------------
# RunMyCrew — VPS deploy
#
# Run from /opt/runmycrew/deploy:
#     ./deploy.sh                  # update to whatever main currently builds
#     RUNMYCREW_IMAGE_TAG=sha-abc1234 ./deploy.sh   # pin a specific build
#
# What it does:
...
echo
echo "✓ RunMyCrew updated."
echo "  Repo HEAD: $(git rev-parse --short HEAD)"
echo "  Image tag: ${RUNMYCREW_IMAGE_TAG:-latest}"
echo
echo "Tail logs with:"
