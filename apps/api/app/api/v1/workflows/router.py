...
_AGENT_NODE_TYPES = {
    "action.agent", "action.llm", "action.a2a", "action.evaluator",
    "action.thinking", "action.browser_use", "action.vision",
}
...
async def list_workflows_with_stats(
    ...
    for w in workflows:
        ...
        next_run  = _compute_next_run(cron_info["cron_expression"], cron_info["timezone"]) if cron_info else None
        result.append({
            "id":              wid,
            "name":            w.name,
