...
async def get_dashboard_stats(
    ...
    wf_ids_result = await db.execute(
        sa.select(Workflow.id).where(Workflow.workspace_id == workspace.id)
    )
    wf_ids = [r[0] for r in wf_ids_result.fetchall()]

    if not wf_ids:
        return _empty_stats(current_user)

    # ── Today's runs ─────────────────────────────────────────────────────────
    runs_today = await _count_executions(db, wf_ids, today_start, now)
    runs_yesterday = await _count_executions(db, wf_ids, yesterday_start, today_start)
    runs_delta_pct = _pct_change(runs_yesterday, runs_today)

    # ── Success rate today ───────────────────────────────────────────────────
    completed_today = await _count_executions(db, wf_ids, today_start, now, status="completed")
    failed_today    = await _count_executions(db, wf_ids, today_start, now, status="failed")
    ...
    completed_yest = await _count_executions(db, wf_ids, yesterday_start, today_start, status="completed")
    failed_yest    = await _count_executions(db, wf_ids, yesterday_start, today_start, status="failed")
    ...
    runs_spark    = await _hourly_counts(db, wf_ids, twelve_ago, now, status=None)
    success_spark = await _hourly_rate(db, wf_ids, twelve_ago, now)
    ...
    recent_runs = [
        {
            ...
            "ago":           _time_ago(r.started_at),
        }
        for r in recent_rows
    ]

    # ── Next 12 hours schedules ───────────────────────────────────────────────
    schedules = await _next_schedules(db, workspace.id, limit=4)
...
def _empty_stats(user: User) -> dict:
    zero_spark = [0] * 12
    return {
        "stats": [
            {"label": "Runs today",       "value": "0",   "unit": "",  "delta": "+0%",  "delta_dir": "flat", "spark": zero_spark},
            {"label": "Success rate",     "value": "100", "unit": "%", "delta": "+0pp", "delta_dir": "flat", "spark": [100] * 12},
            {"label": "Time saved",       "value": "0.0", "unit": "hr","delta": "+0hr", "delta_dir": "flat", "spark": zero_spark},
            {"label": "Active workflows", "value": "0",   "unit": "",  "delta": "+0",   "delta_dir": "flat", "spark": zero_spark},
        ],
        "recent_runs": [],
        "schedules":   [],
        "connections": [],
...
async def _count_executions(db, wf_ids, start, end, status=None) -> int:
...
async def _hourly_counts(db, wf_ids, start, end, status=None) -> list[float]:
...
async def _hourly_rate(db, wf_ids, start, end) -> list[float]:
    ...
    for i in range(12):
        b_start = start + i * bucket_size
        b_end   = b_start + bucket_size
        total = await _count_executions(db, wf_ids, b_start, b_end)
        ok    = await _count_executions(db, wf_ids, b_start, b_end, status="completed")
        result.append(round(ok / total * 100, 1) if total > 0 else 100.0)
    return result


async def _next_schedules(db, workspace_id, limit=4) -> list[dict]:
...
def _fmt_duration(started_at, finished_at) -> str:
    if not started_at:
        return "—"
    if not finished_at:
        return "running"
