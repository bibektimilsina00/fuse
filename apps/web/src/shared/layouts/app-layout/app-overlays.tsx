...
function KeyboardShortcutsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed z-[9999] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[520px] bg-[var(--bg-2)] border border-[var(--border)] rounded-xl shadow-[0_24px_56px_-20px_oklch(0_0_0/0.7)] overflow-hidden">
...
function FeedbackModal({ open, text, sent, onTextChange, onSend, onClose }: FeedbackModalProps) {
  ...
  return createPortal(
    <>
      <div className="fixed inset-0 z-[9998] bg-black/50 backdrop-blur-sm" onClick={onClose} />
      <div className="fixed z-[9999] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] bg-[var(--bg-2)] border border-[var(--border)] rounded-xl p-6 flex flex-col gap-5 shadow-[0_24px_56px_-20px_oklch(0_0_0/0.7)]">
        {sent ? (
          <div className="flex flex-col items-center gap-3 py-4 text-center">
            ...
            <button onClick={onClose} className="mt-2 px-4 py-2 rounded-md bg-[var(--text)] text-[var(--bg)] text-[13px] font-medium border-none cursor-pointer hover:bg-[oklch(0.90_0.003_250)] transition-colors">
              Close
            </button>
          </div>
        ) : (
          <>
            ...
            <textarea
              ...
              rows={5}
              className="w-full bg-[var(--bg)] border border-[var(--border-faint)] rounded-md px-4 py-3 text-[13px] text-[var(--text)] placeholder:text-[var(--text-faint)] outline-none resize-none focus:border-[var(--border)] transition-colors"
            />
