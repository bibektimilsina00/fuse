...
import { useToggleAutomation, useDeleteAutomation, useDuplicateAutomation, useRunAutomation } from '../hooks/useAutomations'
...
export function AutomationList({ items }: Props) {
  ...
  const handleDelete = async (a: Automation, e: React.MouseEvent) => {
    e.stopPropagation()
    const ok = await confirm({ title: 'Delete automation', message: `Delete "${a.name}"? This cannot be undone.`, confirmText: 'Delete', variant: 'danger' })
    if (!ok) return
    await remove.mutateAsync(a.id)
    toast('Deleted', { variant: 'ok', description: a.name })
  }

  const handleDuplicate = async (a: Automation, e: React.MouseEvent) => {
    e.stopPropagation()
    await duplicate.mutateAsync(a.id)
    toast('Duplicated', { variant: 'ok', description: `"${a.name} (copy)" created` })
  }

  const handleRun = async (a: Automation, e: React.MouseEvent) => {
    e.stopPropagation()
    ...
    return (
      <div className="panel">
        <Empty
          ...
          description="No automations match your current filter."
