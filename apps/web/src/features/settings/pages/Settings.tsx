    ...
    <div className="view-body">
      ...
      {newlyCreatedKey && (
        <>
          ...
          />
          <div className="fixed z-[9999] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[480px] bg-[var(--bg-2)] border border-[var(--border)] rounded-xl p-6 flex flex-col gap-5 shadow-[0_24px_56px_-20px_oklch(0_0_0/0.7)]">
            {/* Title */}
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                onClick={() => setNewlyCreatedKey(null)}
                className="w-[28px] h-[28px] rounded-sm flex items-center justify-center text-[var(--text-faint)] hover:bg-[var(--surface)] hover:text-[var(--text)] transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            {/* Warning */}
            <div className="flex items-start gap-3 bg-[oklch(0.82_0.14_80/0.12)] border border-[oklch(0.82_0.14_80/0.3)] px-3.5 py-3 rounded-md">
            ...
            <div className="flex gap-2">
              <div className="flex-1 flex items-center px-3 h-[38px] bg-[var(--bg)] border border-[var(--border-faint)] rounded-md font-mono text-[12px] text-[var(--text-mute)] overflow-hidden">
                <span className="truncate select-all">{newlyCreatedKey}</span>
              </div>
              <button
                onClick={handleCopy}
                className="inline-flex items-center gap-2 px-3.5 h-[38px] rounded-md bg-[var(--surface)] border border-[var(--border-faint)] text-[13px] font-medium text-[var(--text-mute)] hover:bg-[var(--surface-2)] hover:text-[var(--text)] transition-colors shrink-0"
              >
                {copied
            ...
            <div className="flex justify-end pt-1 border-t border-[var(--border-faint)]">
              <button
                onClick={() => setNewlyCreatedKey(null)}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-[var(--text)] text-[var(--bg)] text-[13px] font-medium border-none cursor-pointer hover:bg-[oklch(0.90_0.003_250)] transition-colors"
              >
                I've saved this key
              </button>
            </div>
